...
class RealSenseCamera(Camera):
    ...
    def connect(self, warmup: bool = True) -> None:
        ...
        for attempt in range(1, max_retries + 1):
            ...
            try:
                ...
                logger.warning(f"{self} warmup failed (attempt {attempt}/{max_retries}): {e}")
                # Stop read thread first (before pipeline stop)
                ...
                if attempt < max_retries:
                    # Hardware reset the camera to recover from stuck state
                    logger.info(f"{self} performing hardware reset before retry...")
                    try:
                        ctx = rs.context()
                        ...
                        logger.warning(f"{self} hardware reset failed: {reset_err}")
                    time.sleep(5)
                    logger.info(f"{self} retrying (attempt {attempt + 1}/{max_retries})...")
                else:
                    raise ConnectionError(
                        f"{self} failed to connect after {max_retries} attempts."
                    ) from e
    ...
    def _stop_read_thread(self) -> None:
        ...
        # Set to None only after thread has fully stopped
        self.thread = None
        # Small delay to ensure thread loop has exited before clearing stop_event
        time.sleep(0.1)
