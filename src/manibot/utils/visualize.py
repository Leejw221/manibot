...
def plot_joint_trajectories(pred, gt, dim_names=None):
    """Per-dimension predicted vs. ground-truth action trajectory over the action horizon.

    Args:
        pred, gt: (horizon, action_dim) arrays for a single sample.
        dim_names: optional list of per-dimension labels (defaults to "dim_i").
    """
    horizon, action_dim = gt.shape
    dim_names = dim_names or [f"dim_{i}" for i in range(action_dim)]

    n_cols = 2
    n_rows = -(-action_dim // n_cols)  # ceil
    fig, axes = plt.subplots(n_rows, n_cols, figsize=(4 * n_cols, 2 * n_rows), squeeze=False)
    steps = np.arange(horizon)

    for d in range(action_dim):
        ax = axes[d // n_cols][d % n_cols]
        ax.plot(steps, gt[:, d], "--", color="gray", label="GT")
        ax.plot(steps, pred[:, d], "-", color="tab:blue", label="pred")
    ...
    fig.suptitle("Predicted vs GT action (per dimension)", fontsize=12)
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    return fig


def plot_input_image(image):
    """Sanity-check figure for a single (C, H, W) float[0,1] image tensor (post resize/crop)."""
    fig, ax = plt.subplots(figsize=(4, 3))
    ax.imshow(np.clip(image.transpose(1, 2, 0), 0, 1))
    ax.set_title("Encoder input (post resize/crop)", fontsize=9)
    ax.axis("off")
    plt.tight_layout()
    return fig
