...
class SquareExpert:
    ...
    def _grasp_frame(self):
        ...
        **두 개**이고 파지에는 동등하다 — **지금 손목에서 가까운 쪽**을 골라 회전을 90° 이내로
        ...
        ⚠ 손목 카메라가 너트 쪽을 보는 자세로 **고정하지 않는다**. 그렇게 하면 위 회전이
        최대 180° 가 되어 파지가 무너진다.
    ...
    def _choose_yaw(self):
        ...
        한쪽만 보면 joint7 한계에 걸리는 것처럼 보인다(2026-09-08 에 실제로 그렇게 오판했다).
        [검증 2026-09-08] 20 시행에서 20/20 이 금지 아닌 방향을 찾고 파지도 20/20 유지.
        """
        _, Rn = self._nut()
        phi = _yaw(Rn)
        nut, _ = self._nut()
