...
class Nero(ManipulatorModel):
    ...
    @property
    def init_qpos(self):
        """작업 준비 자세 — 팔꿈치를 내리고 아래팔을 앞으로 모은 사람 같은 자세.

        **원점 자세도 조립 자세도 아니다.** 셋을 구분한다:
          원점 : 전 관절 0° = T자세. 벤더가 정한 것이고 작업엔 못 쓴다.
          조립 : (0, 90, 0,0,0,0,0). 실물이 서 있는 자세 — 기동·종료용.
                 joint2 여유가 9.7° 뿐이라 리셋 자세로 쓰면 안 된다.
          준비 : 여기. 매 에피소드가 여기서 시작하고, 실물도 조립 자세에서 여기로 와서 시작한다.

        robosuite OSC 는 영공간에서 **리셋 자세**를 목표로 당기므로 이 자세의 관절 여유가
        그대로 제어 여유가 된다. 손을 몸 앞(base+0.32, ±0.24, 작업대+0.10)에 두고 손목 roll 을
        자유롭게 푼 해 1000여 개 중, 여유 16° 이상이면서 **팔꿈치가 가장 낮은** 것을 골랐다:
        팔꿈치 z=0.939 (어깨 1.08 보다 14cm 아래) · 여유 17.2° [2026-09-06].

        ⚠️ 팔꿈치를 더 내리긴 어렵다. 기하학상 한계는 0.78 이지만 거기로 가는 해는 joint1 이
        한계(-155°)에 1.1° 남기고 붙는다. 장착 각도를 90° 단위로 바꿔 봐도 네 방향 모두
        팔꿈치 최저가 0.939 로 같아, **재조립으로 해결되는 문제가 아니다**(확인함).
        """
        right = np.radians([47.6, 42.4, 8.5, 105.4, 32.1, 23.7, -66.4])
